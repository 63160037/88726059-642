function formValidation () {
    ...
    function validateUserID(uid, min, max) {
        ...
        return false; } 
            ...
            uid.style.background = 'Yellow'; 
            error = "User ID ต้องมีความยาว " + min + "_" + max + " ตัวอักษร\n"; 
            alert(error); 
            uid. focus();
            return false; } 
...
function validatePassword(passid, min, max) {
    ...
    if (passid.value == "") {
        ...
        error = "Password มีตัวอักรที่ไม่ได้รับอนุญาต\n"; 
        passid.style.background = 'Yellow'; 
        alert(error); 
        passid. focus();
        return false; } 
    ...
    function validateEmail(uemail) {
        var filter = /^([a-zA-Z0-9_\.\])+\@(([a-zA-Z0-91-1) +\.) +([a-zA-Z0-9]{2,4})+$/;
    
        if (!filter.test(uemail.value)) {
            alert( 'Email ไม่ถูกต้อง'); uemail. focus();
            return false; } 
        else {
            return true;
        }
